...
use crate::auth::browser::{
    close_browser, find_browser_paths, launch_browser, set_cookies, Cookie,
};
        ...
        for module in &modules {
            ...
            if modname == "pdfannotator" {
                ...
                let fallback_url = if let Some(ctxid) = contextid {
                    ...
                    format!(
                        "{}/webservice/pluginfile.php/{}/mod_pdfannotator/content/0/{}",
                ...
                let first_file = module
                    .get("contents")
                    .and_then(|c| c.as_array())
                    .and_then(|arr| arr.first());
                let mimetype = first_file
                    .and_then(|f| f.get("mimetype"))
                    .and_then(|v| v.as_str())
                    .map(String::from);
                let filesize = first_file
                    .and_then(|f| f.get("filesize"))
                    .and_then(|v| v.as_u64());
