...
pub async fn get_forum_discussions_api(
    ...
    if let Some(p) = page {
        args.insert("page".to_string(), serde_json::json!(p));
    }
    if let Some(pp) = per_page {
        args.insert("perpage".to_string(), serde_json::json!(pp));
    }
    if let Some(g) = group_id {
        args.insert("groupid".to_string(), serde_json::json!(g));
    }
