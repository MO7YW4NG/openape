...
use super::types::{AssignmentModule, DraftFile, SessionInfo, SubmissionStatus};
...
pub async fn get_submission_status_api(
    ...
    let plugins = submission
        .and_then(|s| s.get("plugins"))
        .and_then(|p| p.as_array())
        .cloned()
        .unwrap_or_default();
    let file_plugin = plugins
        .iter()
        .find(|p| p.get("type").and_then(|v| v.as_str()) == Some("file"));
    ...
    let comments_plugin = feedback
        ...
        .and_then(|plugins| {
            plugins
                .iter()
                .find(|p| p.get("type").and_then(|v| v.as_str()) == Some("comments"))
        })
        .and_then(|cp| cp.get("editorfields").and_then(|ef| ef.as_array()))
        .and_then(|fields| {
            fields
                .iter()
                .find(|f| f.get("name").and_then(|v| v.as_str()) == Some("comments"))
    ...
    Ok(SubmissionStatus {
        submitted: submission
            ...
            == Some("submitted"),
        graded: last_attempt
            .and_then(|la| la.get("gradingstatus"))
            .and_then(|v| v.as_str())
            == Some("graded"),
        grader: feedback
            .and_then(|fb| fb.get("gradername"))
            .and_then(|v| v.as_str())
            .map(String::from),
        grade: feedback
            .and_then(|fb| fb.get("gradefordisplay"))
            .and_then(|v| v.as_str())
            .map(String::from),
        feedback: comments_plugin,
        last_modified: submission
            .and_then(|s| s.get("timemodified"))
            .and_then(|v| v.as_i64()),
        extensions,
    })
}
