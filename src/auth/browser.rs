...
pub async fn launch_browser(
    ...
    let mut config_builder = BrowserConfig::builder().chrome_executable(executable_path);
    ...
    if let Some(data_dir) = user_data_dir {
        ...
        let abs_path = std::fs::canonicalize(data_dir).unwrap_or_else(|_| data_dir.clone());
        ...
        let unique_dir =
            std::env::temp_dir().join(format!("openape-browser-{}", uuid::Uuid::new_v4()));
    ...
    // Mask headless browser indicators to bypass automation detection
    let mask_js = r#"
        Object.defineProperty(navigator, 'webdriver', {get: () => undefined});
        Object.defineProperty(navigator, 'plugins', {get: () => [1, 2, 3, 4, 5]});
        window.chrome = { runtime: {} };
    "#;
    let _ = page.evaluate_on_new_document(mask_js).await;
