...
use crate::moodle::assignment::{
    get_assignments_by_courses_api, get_submission_status_api, save_submission_api,
};
...
pub async fn run(cmd: &crate::AssignmentsCommands, cli: &Cli) -> Result<()> {
    ...
    match cmd {
        crate::AssignmentsCommands::List { course_id } => {
            let assignments =
                get_assignments_by_courses_api(&ctx.client, &ctx.session, &[*course_id]).await?;
        ...
        crate::AssignmentsCommands::ListAll { level } => {
            ...
            let assignments =
                get_assignments_by_courses_api(&ctx.client, &ctx.session, &course_ids).await?;
            ...
            ctx.log.info(&format!(
                "Found {} assignments across {} courses",
                items.len(),
                courses.len()
            ));
            format_and_output(&items, ctx.output, None);
        ...
        crate::AssignmentsCommands::Status { assignment_id } => {
            let status =
                get_submission_status_api(&ctx.client, &ctx.session, *assignment_id).await?;
        ...
        crate::AssignmentsCommands::Submit {
            assignment_id,
            text,
            file_id,
            file,
            ...
            let effective_file_id = if let Some(path) = file {
                ...
                let draft_id =
                    upload_file_api(&ctx.client, &ctx.session, &path_str, None, None, None).await?;
                ctx.log
                    .success(&format!("Uploaded file, draft ID: {}", draft_id));
                Some(draft_id)
            } else {
                *file_id
            };
