...
pub async fn run(cmd: &crate::GradesCommands, cli: &Cli) -> Result<()> {
    ...
    match cmd {
        crate::GradesCommands::Summary => {
            ...
            for course in &courses {
                ...
                .await
                {
                    ...
                    Err(e) => {
                        ctx.log.warn(&format!(
                            "Failed to get grades for course {}: {}",
                            course.id, e
                        ));
                    }
                }
        ...
        crate::GradesCommands::Course { course_id } => {
            ...
            let mut grade =
                get_course_grades_api(&ctx.client, &ctx.session, *course_id, ctx.session.user_id)
                    .await?;
            ...
            ctx.log
                .info(&format!("Found {} grade items", rows.len() - 1));
