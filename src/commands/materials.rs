use super::{in_progress_all_to_classification, level_to_classification, ApiCtx};
use crate::moodle::course::get_enrolled_courses_api;
use crate::moodle::material::{
    get_course_contents_resources, get_incomplete_completions, resolve_pdfannotator_urls,
    view_resource_api,
};
...
pub async fn run(cmd: &crate::MaterialsCommands, cli: &Cli) -> Result<()> {
    ...
    match cmd {
        ...
        crate::MaterialsCommands::ListAll { level } => {
            ...
            let course_map: std::collections::HashMap<u64, &str> = courses
                .iter()
                .map(|c| (c.id, c.fullname.as_str()))
                .collect();

            let items: Vec<serde_json::Value> = all_resources
                .iter()
                .map(|r| {
                    serde_json::json!({
                        "course_id": r.course_id,
            ...
            ctx.log.info(&format!(
                "Found {} materials across {} courses",
                items.len(),
                courses.len()
            ));
            format_and_output(&items, ctx.output, None);
        ...
        crate::MaterialsCommands::Download {
            course_id,
            output_dir,
        } => {
            let mut resources =
        ...
        crate::MaterialsCommands::Complete { course_id, dry_run } => {
            let incompletes = get_incomplete_completions(
                &ctx.client,
                &ctx.session,
                *course_id,
                ctx.session.user_id,
            ...
            let incompletes: Vec<_> = incompletes
                .into_iter()
                .filter(|i| i.modname != "supervideo")
                .collect();
            let total = incompletes.len();
            ctx.log
                .info(&format!("Found {} incomplete resources", total));
            ...
            for i in &incompletes {
                let success = match i.rule.as_deref() {
                    Some("completionview") if i.modname == "resource" => {
                        ...
                        view_resource_api(&ctx.client, &ctx.session, i.instance)
                            .await
                            .unwrap_or(false)
                    }
                    _ => {
                        // Try manual completion API
                        match update_completion_status(&ctx.client, &ctx.session, i.cmid, true)
                            .await
                        {
                            Ok(true) => true,
            ...
            ctx.log
                .info(&format!("Completed {}/{} resources", completed, total));
            let result = serde_json::json!({
                "action": "complete",
                "course_id": *course_id,
                "total": total,
        ...
        crate::MaterialsCommands::CompleteAll { dry_run, level } => {
            ...
            for course in &courses {
                if let Ok(items) = get_incomplete_completions(
                    &ctx.client,
                    &ctx.session,
                    course.id,
                    ctx.session.user_id,
            ...
            ctx.log.info(&format!(
                "Found {} incomplete resources across {} courses",
                total,
                courses.len()
            ));
...
async fn resolve_pdfannotator_in_resources(
    ...
    let pdfannotators: Vec<(String, String, String)> = resources
        .iter()
    ...
    log.info(&format!(
        "Resolving {} pdfannotator URL(s) via headless browser...",
        pdfannotators.len()
    ));
        ...
        match ctx.client.get(&url).send().await {
            Ok(resp) if resp.status().is_success() => {
                ...
                if filename.ends_with(".pdf") && !bytes.starts_with(b"%PDF") {
                    ...
                    ctx.log
                        .success(&format!("  Downloaded: {} ({})", filename, size));
            ...
            Ok(resp) => {
                ...
                ctx.log
                    .warn(&format!("  HTTP {} for: {}", status, resource.name));
                summary.files.push(serde_json::json!({
                    "name": resource.name,
                    "status": "failed",
