...
use crate::moodle::forum::{
    add_discussion_api, add_discussion_post_api, delete_post_api, get_discussion_posts_api,
    get_forum_discussions_api, get_forums_api,
};
...
pub async fn run(cmd: &crate::ForumsCommands, cli: &Cli) -> Result<()> {
    ...
    match cmd {
        ...
        crate::ForumsCommands::Discussions { forum_id } => {
            ...
            let items: Vec<serde_json::Value> = discussions
                .iter()
                .map(|d| {
                    serde_json::json!({
                        "id": d.id,
            ...
            ctx.log
                .info(&format!("Forum {}: {} discussions", forum_id, items.len()));
            format_and_output(&items, ctx.output, None);
        }

        crate::ForumsCommands::Posts { discussion_id } => {
            let posts = get_discussion_posts_api(&ctx.client, &ctx.session, *discussion_id).await?;
        ...
        crate::ForumsCommands::Delete { post_id } => {
            ...
            if ok {
                ctx.log
                    .success(&format!("Post {} deleted successfully!", post_id));
                let result = serde_json::json!({
                    "action": "delete",
                    "post_id": *post_id,
                    "success": true,
...
async fn list_forums(ctx: &ApiCtx, classification: &str) -> Result<()> {
    ...
    let items: Vec<serde_json::Value> = forums
        .iter()
        .filter_map(|f| {
            let course_id = f
                .get("course")
